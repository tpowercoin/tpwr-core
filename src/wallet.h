...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 TPWR at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zTPWR send
enum ZerocoinSpendStatus {
    Ztpwr_SPEND_OKAY = 0,                            // No error
    Ztpwr_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    Ztpwr_WALLET_LOCKED = 2,                         // Wallet was locked
    Ztpwr_COMMIT_FAILED = 3,                         // Commit failed, reset status
    Ztpwr_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    Ztpwr_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    Ztpwr_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    Ztpwr_TRX_CREATE = 7,                            // Everything related to create the transaction
    Ztpwr_TRX_CHANGE = 8,                            // Everything related to transaction change
    Ztpwr_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    Ztpwr_INVALID_COIN = 10,                         // Selected mint coin is not valid
    Ztpwr_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    Ztpwr_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    Ztpwr_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    Ztpwr_SPENT_USED_Ztpwr = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void SetNull()
    {
        ...
        nHashInterval = 22;
        nStakeSetUpdateTime = 360; // 6 minutes
    ...
    void setZtpwrAutoBackups(bool fEnabled)
    {
