...
#include "ui_ztpwrcontroldialog.h"
...
std::list<std::string> ZtpwrControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZtpwrControlDialog::listMints;

ZtpwrControlDialog::ZtpwrControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZtpwrControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZtpwrControlDialog::~ZtpwrControlDialog()
{
    delete ui;
}

void ZtpwrControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZtpwrControlDialog::updateList()
{
...
void ZtpwrControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZtpwrControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZtpwr_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZtpwrControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZtpwrControlDialog::GetSelectedMints()
{
...
void ZtpwrControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
