                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zTPWR ought to be enough for anybody." - Bill Gates, 2017
    ui->zTPWRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzTPWRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzTPWR->setEnabled(false);
        ui->pushButtonMintzTPWR->setToolTip(tr("zTPWR is currently disabled due to maintenance."));

        ui->pushButtonSpendzTPWR->setEnabled(false);
        ui->pushButtonSpendzTPWR->setToolTip(tr("zTPWR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zTPWRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzTPWR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTPWR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zTPWR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zTPWR in ") +
...
void PrivacyDialog::on_pushButtonSpendzTPWR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zTPWR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zTPWR
        sendzTPWR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zTPWR
    sendzTPWR();
}

void PrivacyDialog::on_pushButtonZtpwrControl_clicked()
{
    ZtpwrControlDialog* zTPWRControl = new ZtpwrControlDialog(this);
    zTPWRControl->setModel(walletModel);
    zTPWRControl->exec();
}

void PrivacyDialog::setZtpwrControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzTPWRSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzTPWR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid TPWR Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zTPWRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zTPWR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " TPWR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zTPWR</b>";
    ...
    // use mints from zTPWR selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZtpwrControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZtpwrControlDialog::GetSelectedMints();
    }
    ...
    // Clear ztpwr selector in case it was used
    ZtpwrControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zTPWR Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " TPWR, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zTPWR Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zTPWR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zTPWR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zTPWR "));
    ui->labelzTPWRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzTPWRSyncStatus->setVisible(fShow);
}
