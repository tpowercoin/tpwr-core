...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::TPWR, nValue);
...
{
    std::string genesisblockhash = "0x00000224b41a6f29aa84bb68928295b63b9ace770a571fd0b6be53bf7dae1013";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (tpwr.conf).");
            QMessageBox::warning(this, "TPWR Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
