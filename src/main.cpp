...
#if defined(NDEBUG)
#error "TPWR cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zTPWR spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zTPWR spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zTPWR spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= 1) {
        ...
        nSubsidy = 0.5 * COIN;
    } else if (nHeight >    9999 && nHeight <=   50000) {
        nSubsidy = 2.0 * COIN;
    } else if (nHeight >   50000 && nHeight <=  250000) {
        nSubsidy = 0.8 * COIN;
    } else if (nHeight >  250001 && nHeight <=  500000) {
        nSubsidy = 1.0 * COIN;
    } else if (nHeight >  500001 && nHeight <=  750000) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight >  750001 && nHeight <= 1000000) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight > 1250001 && nHeight <= 1500000) {
        nSubsidy = 2.2 * COIN;
    } else if (nHeight > 1500001 && nHeight <= 1750000) {
        nSubsidy = 2.6 * COIN;
    } else if (nHeight > 1750001 && nHeight <= 2000000) {
        nSubsidy = 3.0 * COIN;
    } else if (nHeight > 2000001 && nHeight <= 2250000) {
        nSubsidy = 2.6 * COIN;
    } else if (nHeight > 2250001 && nHeight <= 2500000) {
        nSubsidy = 2.2 * COIN;
    } else if (nHeight > 2500001 && nHeight <= 2750000) {
        nSubsidy = 1.8 * COIN;
    } else if (nHeight > 2750001 && nHeight <= 3000000) {
        nSubsidy = 1.4 * COIN;
    } else if (nHeight > 3000001 && nHeight <= 3250000) {
        nSubsidy = 1.0 * COIN;
    } else if (nHeight > 3250001 && nHeight <= 3500000) {
        nSubsidy = 0.6 * COIN;
    } else if (nHeight > 3500001) {
        nSubsidy = 0.2 * COIN;
    }
    else {
        nSubsidy = 0.1 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from TPWR
...
{
    RenameThread("tpwr-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZtpwrMinted()
{
...
void RecalculateZtpwrSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zTPWR supply
...
bool RecalculatetpwrSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zTPWR with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zTPWRSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert TPWR to zTPWR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zTPWR spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zTPWR serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zTPWR mints and %d zTPWR spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // TPWR: We use certain sporks during IBD, so check to see if they are
